...
public class CleanGuitarSynth implements ISynthesizer
{
    ...
    public CleanGuitarSynth(int mappedChannel)
    {
        try
        {
            ...
            {
                if (insts[i].toString().startsWith("Instrument: Clean Gt."))
                {
                    assert channel != null;
        ...
        System.out.println("Clean Guitar loaded on channel " + mappedChannel);
    }
